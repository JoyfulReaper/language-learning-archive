...
type Validation<'Sucess, 'Failure> =
    Result<'Sucess, 'Failure list>
...
module Validation =
    ...
    /// Apply a Validation<fn> to a Validation<x> applicativly
    ...
    let sequence (aListOfValidations:Validation<_,_> list) =
        ...
        // loop through the list, prepending eash element
        // to the initial value
...
module AsyncResult =
    ...
    /// Apply ignore to then internal value
    let ignore x =
        x |> map ignore
    ...
    let sleep ms = 
        (Async.Sleep : int -> Async<unit>) ms |> ofAsync
