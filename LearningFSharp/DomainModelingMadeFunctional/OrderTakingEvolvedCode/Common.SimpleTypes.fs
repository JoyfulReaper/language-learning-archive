...
/// An Id for Orders. Constrained to be an non-empty string < 10 char
...
module String50 =
    ...
    /// Return the value inside the String50
...
module EmailAddress =
    ...
    let create fieldName str =
        let pattern = ".+@.+" // Anything seperated by an "@"
...
module Price =
    ...
    /// Return the value inside Price
    let value (Price v) = v

    /// Create a Price from a decimal
    /// Return Error if input is not a decimal between 0.0 and 1000.0
    let create v =
    ...
    /// Multiple a Price by a decimal qty.
...
module BillingAmount =
    ...
    /// Return the value inside BillingAmount
